...
class SunPosition:
    def __init__(self, lat, lon, date_time, isDST):
        ...
        self.isDST = isDST #is Daylight Saving Time   
    ...
    def sun_position(self):
        ...
        # TODO add here daylight saving time parameter
        ...
        #day_number = self.__day_number(year, month, day)
        julianday = self.__julianday(year, month, day)
        print("julianday: " + str(julianday))
        epsilon_g, omega_g, e = self.__orbital_elems(julianday)
        mean_anomaly = self.__mean_anomaly(julianday, epsilon_g, omega_g)
        true_anomaly = self.__true_anomaly(e, mean_anomaly)
        ecliptic_lon = self.__torange(true_anomaly + omega_g)
        right_asc, declination = self.__equatorial_coords(ecliptic_lon)
        #altitude, azimuth = self.__horizon_coords(declination, right_asc, 
                                                  #self.lat, self.lon,
                                                  #year, month, day, UT)
        
        #print("year, month, day, UT: " + str(year) + "," + str(month) + "," + str(day) + "," + str(UT))
        ...
        print("ecliptic_lon: " + str(ecliptic_lon))
        print("declination: " + str(declination))
        print("right_asc: " + str(right_asc))        
        #print("altitude: " + str(altitude))
        #print("azimuth: " + str(azimuth))

    #def __julianday(self, year, month, day, UT):
        #""""Calculates Julian Day. UT is universal time"""
        #month_ = month
        #year_ = year
        ...
        #julian_day = (int(365.25*(year_ + 4716)) + int(30.6001*(month_ + 1)) +
                      #d_day + b - 1524.5)
    ...
    #def __julianday(self, year, month, day, UT):
        ...
        #if year_ < 0:
            #c = int((365.25*year_) - 0.75)
        #else:
            #c = int(365.25*year_)
        
        #d = int(30.6001*(month_ + 1))
    
        #return b + c + d + d_day + 1720994.5
        
    def __julianday(self, year, month, day):
        """"Calculates Julian Day for UT. Day is a decimal number becuase it 
        includes time of the day. Only Gregorian dates."""
        if month > 2:
            y = year
            m = month
        else:
    ...
    def __orbital_elems(self, julianday):
    ...
    def __equatorial_coords(self, ecliptic_lon):
        ...
        # TODO adjust for a standard epoch for all calculations, wheter 2000 or 2010
        julianday = self.__julianday(2009, 12, 31)
    ...
    def __horizon_coords(self, declination, right_asc, latitude, longitude,
        ...
        julianday = self.__julianday(year, month, day)        
        GST = self.__UT2GST(year, month, day, UT)
    ...
    def __LCT2UT_(self, dt_obj, isDST, longitude):
    ...
    def __LCT2UT(self, dt_obj, isDST, longitude):
        ...
        #day_ = int(g_day_)
        #UT = (g_day_ - int(g_day_))*24
        
        print("year, month, day: " + str(year) + "," + str(month) + "," +str(day_))

        return year_, month_, day_

    
    def __UT2GST(self, year, month, day, UT):
    ...
    def __torange(self, angle):
        """Bring angle to range 0-360"""
        # TODO: review and test for other values
        if angle > 360:
            angle = angle - 360
        elif angle < 0:
            angle = angle + 360
        if angle > 360 or angle < 0:
            logging.error("Cannot bring angle into range o to 360")
        else:
            return angle
